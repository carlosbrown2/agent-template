...
- **Tunable autonomy** — Confidence routing is one bit derived from the verification gate (`PASS → HIGH`, anything else → `LOW`). The `auto-land:` policy maps that bit to "run the post-bead landing ritual and push", "pause on LOW", or "always pause", letting you dial human oversight from full (`auto-land: none`) to zero (`auto-land: all`). The agent self-escalates when it's stuck.
